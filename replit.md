# Dynamic Offshore Drilling Inc

A comprehensive corporate website and admin management system — featuring a public-facing website, worker self-service portal, and a full admin dashboard for HR management.
...
- `lib/db/src/schema/` — Drizzle ORM schema definitions (workers, jobs, leaveTypes, leaveRequests, leaveLetters, admins)
...
- Database: Supabase PostgreSQL via `SUPABASE_DATABASE_URL` secret. The `lib/db/src/index.ts` also falls back to `DATABASE_URL`. SSL is enabled with `rejectUnauthorized: false`.
...
**Public website**: Home, About Us, Careers (live job listings with filter + expand), Contact (global offices + enquiry form), Services, Sustainability, Newsroom, Investors, Oil Rigs pages.
...
**Admin Dashboard** (`/admin/login`, super admin credentials set via `ADMIN_USERNAME`/`ADMIN_PASSWORD` env vars, default: `admin` / `Dynamic Offshore Drilling@2024`):
...
- The vite dev server proxies `/api` requests to `localhost:8080` (the API server).
